...
export function Card({ card, size = "md" }: CardProps) {
  ...
  return (
    <div
      className={`flex ${sizeMap[size]} flex-col justify-between overflow-hidden rounded-[1.35rem] bg-white p-2.5 shadow-[0_10px_25px_rgba(0,0,0,0.2)]`}
    >
      ...
      <div
        className={`rotate-180 self-end font-mono text-sm font-bold leading-tight ${suit.color}`}
      >
        <div>{rank}</div>
        <div>{suit.symbol}</div>
      </div>
